...
v3.8 이전(v3.6/v3.7)으로 만든 한글본에서 발생합니다. 게임은 모드 로컬라이제이션을 **`.loca` 바이너리**로 읽는데, 해당 버전이 `.loca`를 만들지 않고 `.xml`만 남겨 게임이 텍스트를 못 읽은 것입니다.

- **새로 번역/검수**: v3.8부터는 번역·검수 산출물에 `.loca`가 자동 포함됩니다. 해당 모드를 v3.8로 다시 처리하면 해결됩니다.
- **이미 설치된 한글본 일괄 복구(재번역 불필요)**: `bg3_repair_notfound.py`가 Mods 폴더의 한글본을 스캔해 `.loca`를 재생성·재패킹합니다. 원본은 `Mods_backup_<날짜>`에 자동 백업됩니다.
  ```bash
  # 먼저 점검 (쓰기 없음)
  python bg3_repair_notfound.py --mods "<Mods 폴더>" --divine "<Divine.exe 경로>" --dry-run
  # 실제 복구
  python bg3_repair_notfound.py --mods "<Mods 폴더>" --divine "<Divine.exe 경로>"
  ```
...
| `bg3_repair_notfound.py` | 기존 한글본의 누락된 `.loca` 재생성 일괄 복구 (Not Found 해결) |
...
# 기존 한글본의 .loca 일괄 복구 (Not Found 해결) — 먼저 --dry-run 권장
python bg3_repair_notfound.py --mods "<Mods 폴더>" --divine "<Divine.exe 경로>" --dry-run
```
