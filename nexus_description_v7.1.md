...
I have a full-time day job, so I can't update this tool as often as I'd like — sorry in advance for slow fixes and replies.
Please also understand that this tool will probably never translate every mod. Mods are structured in many different ways, and I don't fully understand every mechanism of the game or of modding. I'm building this through "vibe coding" — with a lot of AI help — while learning as I go. Some mods will fail or come out wrong, and I appreciate your patience. Reports (with the mod name and the red log lines) are the best way to help me improve it.
...
작성자의 한마디
저는 본업이 있는 직장인이라 업데이트를 자주 하지 못합니다. 수정과 답변이 늦더라도 양해 부탁드립니다.
그리고 솔직히 말씀드리면, 이 툴이 모든 모드를 번역할 수는 없을 것입니다. 모드마다 구조가 다르고, 저 역시 게임과 모드의 모든 메커니즘을 파악하지 못한 상태에서 AI의 도움을 받아 바이브코딩으로 만들어 가고 있습니다. 어떤 모드는 실패하거나 결과가 이상할 수 있습니다. 이런 현실을 너그럽게 이해해 주시면 감사하겠습니다. 모드 이름과 빨간 로그 줄을 함께 제보해 주시는 것이 툴을 개선하는 가장 큰 도움입니다.
