...
v3.8 이전(v3.6/v3.7)으로 만든 번역본에서 발생합니다. 게임은 모드 로컬라이제이션을 **`.loca` 바이너리**로 읽는데, 해당 버전이 `.loca`를 만들지 않고 `.xml`만 남겨 게임이 텍스트를 못 읽은 것입니다.

- **새로 번역/검수**: v3.8부터는 번역·검수 산출물에 `.loca`가 자동 포함됩니다. 해당 모드를 최신 버전으로 다시 처리하면 해결됩니다.
- **이미 설치된 번역본 일괄 복구(재번역 불필요)**: `bg3_repair_notfound.py`가 Mods 폴더의 번역본을 스캔해 `.loca`를 재생성·재패킹합니다. 원본은 `Mods_backup_<날짜>`에 자동 백업됩니다.
  ```bash
  # 먼저 점검 (쓰기 없음)
...
### MCM(Mod Configuration Menu) 의존 모드 번역 — v3.5 신규
...
- **블루프린트 평문 필드** — `ModName`, `TabName`, `SectionName`, `Name`, `Description`, `Tooltip`, `Label`, `Choices` 등을 번역해 직접 치환합니다. 단, 같은 노드에 `NameHandle`/`DescriptionHandle`/`TooltipHandle`/`LabelHandle` 같은 `Handles`가 정의돼 있으면 건드리지 않고 Localization XML 측 번역에 맡깁니다(게임은 핸들 기반 텍스트를 우선 사용).
...
- **비표준 평면 Localization** — `Localization/Test.xml`처럼 언어 서브폴더 없이 평면 구조를 사용하는 모드(예: Tooltip Manager)도 자동 감지해 in-place로 번역합니다.

- **다국어 폴더 미러** — 번역본을 만든 뒤 동명 파일이 존재하는 다른 언어 폴더에도 번역을 복사합니다. 일부 BG3MCM이 게임 언어 설정과 무관하게 특정 폴더의 XML만 읽는 경우에 대비한 안전장치입니다.

> 검수 큐 리포트 `<modname>_mcm_review.json`은 자동 처리에서 제외된 항목 목록을 제공합니다. 필요하면 직접 편집해 라벨 맵을 작성하거나 추가 번역 작업의 기준으로 사용할 수 있습니다.
...
| 파일 | 설명 |
|------|------|
| `bg3_mod_translator.py` | GUI 실행 진입점 |
| `bg3_repair_notfound.py` | 기존 번역본의 누락된 `.loca` 재생성 일괄 복구 (Not Found 해결) |
...
# 기존 번역본의 .loca 일괄 복구 (Not Found 해결) — 먼저 --dry-run 권장
python bg3_repair_notfound.py --mods "<Mods 폴더>" --divine "<Divine.exe 경로>" --dry-run
```
...
python -m PyInstaller bg3_mod_translator.spec
# 결과물: dist/BG3_ModTranslator.exe
```
...
### v3.7.2
- **`CONTENT_BLOCK_RE`의 self-closing 매칭 누락 수정**: `<content ... />` 같은 빈 self-closing 핸들이 직후의 정상 블록과 한 블록으로 합쳐져 번역 위치가 어긋나던 버그. 정규식의 `[^>]*` → `[^/>]*`로 self-closing의 `/`를 attribute 영역에서 제외해 별도 매칭. 단위 테스트 4건 추가.

### v3.7.1
- **raw `&` 자동 escape**: v3.6/v3.6.1/v3.7의 escape 함수는 `<`/`>`만 처리하고 `&`는 누락. 이제 `&`도 `&amp;` entity로 자동 변환. valid entity(`&lt;`, `&gt;`, `&amp;`, `&#...;` 등)는 보존.

### v3.7
- **`.loca`/`.loca.xml` 정리, `.xml`만 패킹**: 패킹 직전에 모든 `.loca` 바이너리와 `.loca.xml` 보조 파일을 정리합니다. `.xml` 짝이 없는 `.loca.xml`은 `.xml`로 rename. 결과 PAK엔 `.xml`만 남아 게임이 정확히 해당 언어 폴더의 번역을 읽습니다.
...
### v3.6.1
- **번역 캐시 적중 시에도 escape 적용**: v3.6의 raw `<>` 자동 escape가 Gemini API 호출 결과에만 적용됐고, 번역 캐시 적중·글로서리 적중 경로엔 누락. 이제 모든 경로(캐시·글로서리·API·최종 출력 블록)에 escape를 적용해 어디서 결과가 들어와도 안전합니다.

### v3.6
- **번역 결과 XML 깨짐 자동 복구**: Gemini가 D&D 용어를 번역하며 raw `<>`를 결과에 포함시키는 경우를 자동으로 `&lt;`/`&gt;` entity로 변환합니다 (원본의 valid entity는 그대로 보존).
- **Gemini 시스템 프롬프트 보강**: 번역 결과에 `<...>` 형태의 새 태그나 placeholder를 만들지 않도록 규칙 추가.
- **`.loca` 변환 실패 시 진단 로그**: divine의 stderr를 로그에 표시.
- 단위 테스트 6건 추가(`tests/test_xml_escape.py`).

### v3.5.2
- **미러를 prefix 기반으로 확장**: 원본 PAK에 `english.xml`과 `english.loca`가 모두 들어있는 모드는 베이스 prefix(`english`) 기반 매칭으로 영어 폴더의 `english.xml`·`english.loca.xml`을 한 번에 모두 번역으로 덮어씁니다.

### v3.5.1
- **`.loca` 역변환 누락 수정**: 일부 모드가 `*.loca.xml`이 아닌 `english.xml` 이름을 사용하는 케이스 처리. 이제 `Localization/` 안의 모든 `.xml`을 `.loca`로 역변환합니다.

### v3.5
- **MCM(Mod Configuration Menu) 의존 모드 자동 처리**: `MCM_blueprint.json` 평문 필드 및 `ScriptExtender/Lua/*.lua` 안의 IMGUI 위젯 호출 자동 번역.
- **`.loca` 바이너리 역생성**: 번역한 `.loca.xml`을 `.loca` 바이너리로 역변환해 PAK에 포함.
