...
설정 파일이 없을 때 API키·Divine·BG3 경로를 한 번에 안내·입력받는다.
...
def test_wizard_prefills_from_config(qapp):
    ...
    cfg = UserConfig(divine_exe_path="D:/Divine.exe", bg3_install_path="D:/BG3")
    w = FirstRunWizard(cfg)
    assert w._divine_picker.get() == "D:/Divine.exe"
