...
STRINGS = {
    ...
    "settings.bg3_path": "발더스 게이트 3 설치 폴더",
    ...
    "settings.use_official_glossary": "공식 언어팩 참조",
    "settings.save": "저장",
    ...
    "desc.settings.bg3": "발더스 게이트 3가 설치된 폴더(…\\Baldurs Gate 3)입니다. 게임 설정 언어 자동 감지와 공식 용어 참조에 사용됩니다. Steam이면 보통 자동으로 채워집니다.",
    ...
    "desc.settings.mcm": "MCM 설정 메뉴에 의존하는 모드의 블루프린트·Lua 텍스트까지 자동으로 번역합니다. 일반 모드에는 영향이 없으니(자동 감지) 켜둔 채로 두면 됩니다.",
    "desc.settings.official": "설치된 게임의 공식 언어팩을 번역 기준으로 활용합니다. 모드 텍스트가 공식 용어와 정확히 일치하면 공식 번역을 그대로 재사용해(API 절약) 일관성을 높이고, 문장 속 공식 용어는 AI에 함께 전달해 표기를 통일합니다. 위의 BG3 설치 폴더가 필요합니다. 사전은 내 로컬 게임 파일에서 한 번만 만들어 캐시하며, 외부로 재배포하지 않습니다.",
    ...
    "desc.review.edit": "원문과 번역을 표 형태로 나란히 비교합니다. 오른쪽 번역 칸을 더블클릭해 직접 수정하면 해당 항목이 수정됨으로 표시(금색)됩니다.",
    ...
    "glossary.custom_info": "내 용어집이 기본 용어집보다 우선 적용됩니다. 칸을 더블클릭해 바로 입력·수정하고, 맨 아래 빈 줄에 새 용어를 추가하세요. 입력하면 자동으로 저장됩니다.",
