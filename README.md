...
**Q. 한글화했는데 게임에서 여전히 영어로 나와요**
일부 모드는 텍스트를 `Localization` 폴더가 아닌 다른 곳(스크립트, 다이얼로그 파일 등)에 직접 넣어놓습니다. 이런 모드는 `Korean` 폴더에 번역 파일을 만들어도 게임이 그 파일을 읽지 않습니다. 이 경우 이 스크립트로는 한글화가 불가능하며, 모드 제작자가 텍스트를 `Localization` 구조로 옮겨야만 번역이 적용됩니다.
