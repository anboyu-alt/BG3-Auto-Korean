...
| 파일 | 설명 |
|------|------|
| `BG3_AutoKorean_PAK_v2.1.py` | **PAK 모드** — .pak 파일을 넣으면 전자동 번역 |
| `BG3_AutoKorean_Folder_v2.1.py` | **폴더 모드** — 이미 풀어놓은 모드 폴더를 번역 |
| `BG3_AutoKorean_Reviewer_v2.1.py` | **번역 검수 도구** — 한글화된 .pak의 번역을 검수하고 수정 |
...
- **번역 결과를 검수/수정하고 싶으면** → `BG3_AutoKorean_Reviewer_v2.1.py` (검수 도구)
...
> **참고:** 검수 도구(`BG3_AutoKorean_Reviewer_v2.1.py`)도 `divine.exe`를 사용합니다. PAK 모드를 이미 설정했다면 추가 준비 없이 바로 사용 가능합니다.
...
## 사용법: 번역 검수 도구 (`BG3_AutoKorean_Reviewer_v2.1.py`)
...
`BG3_AutoKorean_Reviewer_v2.1.py`를 메모장으로 열어서 `DIVINE_EXE`만 설정합니다:
...
**방법 1: 드래그 앤 드롭 (가장 간편)**
검수할 `.pak` 파일을 `BG3_AutoKorean_Reviewer_v2.1.py` 위에 끌어다 놓으면 바로 실행됩니다.
...
**방법 3: 터미널에서 실행**
```
python BG3_AutoKorean_Reviewer_v2.1.py
```
...
```
══════════════════════════════════════════════════════
  번역 검수 도구 v2.1  |  ModName.xml  |  3 / 127
══════════════════════════════════════════════════════
...
### v2.1 (2025.03)
- **번역 검수 도구 추가** (`BG3_AutoKorean_Reviewer_v2.1.py`): 영어/한국어 비교 검수 및 수정 기능
