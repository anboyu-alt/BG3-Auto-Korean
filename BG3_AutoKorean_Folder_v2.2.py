...
# =============================================================================
# [DEPRECATED] BG3 모드 자동 한글화 스크립트 v2.2 — 폴더 모드
# =============================================================================
#
# ⚠️ 이 파일은 더 이상 권장하지 않습니다.
#    GUI 버전(bg3_autokorean_gui.py)에서 같은 기능을 더 편리하게 제공합니다.
#    BG3 Modder's Multitool 없이 PAK 파일을 직접 처리합니다.
#
# =============================================================================
# BG3 모드 자동 한글화 스크립트 v2.2 — 폴더 모드
# =============================================================================
#
# 이 스크립트는 BG3 Modder's Multitool로 언팩된 모드 폴더를 번역합니다.
# pak 파일을 직접 처리하는 버전은 BG3_AutoKorean_PAK_v2.2.py를 참고하세요.
#
# 두 버전의 차이:
#   - 이 파일 (v2.2, 폴더 모드): 이미 언팩된 폴더를 대상으로 번역만 수행
#     → BG3 Modder's Multitool로 직접 언팩/리팩해야 합니다
#   - BG3_AutoKorean_PAK_v2.2.py (pak 모드): .pak 파일을 넣으면 전자동
...
# ==========================================
# [고급 설정] 기본값으로도 잘 작동하지만, 필요하면 수정 가능합니다
# ==========================================
# INPUT_GLOB: 번역할 파일 확장자 패턴 (기본 *.xml)
# MAX_TOKENS_PER_CHUNK: API에 한 번에 보내는 텍스트 양 (너무 크면 실패율 증가)
# DOWNSHIFT_TOKEN_STEPS: 번역 실패 시 자동으로 줄여보는 청크 크기 목록
# MODELS_TO_TRY: 사용할 AI 모델 목록 (첫 번째를 우선 사용, 실패 시 다음 모델로)
#   → 다른 Gemini 모델을 쓰고 싶으면 여기에 모델명을 추가/변경하세요
# TIMEOUT_SEC: API 응답 대기 시간 (초)
# ==========================================
INPUT_GLOB = "*.xml"
...
# ==========================================
# [용어집] 고유 용어 선번역 목록 — 자유롭게 추가/수정 가능!
# ==========================================
# 형식: "영문 원문": "한국어 번역어"
#
# 이 용어집은 두 가지 방식으로 적용됩니다:
#   1. AI 프롬프트에 포함 → Gemini가 번역할 때 우선 참고
#   2. 번역 결과에 후처리 → 코드가 한 번 더 치환하여 확실하게 적용
#
# 새 모드에 고유명사가 있으면 여기에 추가하세요!
# 예시: "Elminster": "엘민스터" 처럼 한 줄 추가하면 됩니다.
#
# ※ 단어 하나짜리 텍스트(예: 주문 이름)가 용어집에 있으면
#   API를 호출하지 않고 바로 치환합니다 (속도/비용 절감)
# ==========================================
GLOSSARY = {
    # ──────────────────────────────────────
    # 동료 캐릭터
...
# ==========================================
# [번역 캐시] 이미 번역한 텍스트를 저장하여 재사용
# ==========================================
# 한 번 번역된 문장은 translation_cache.json에 저장됩니다.
# 다음 실행 때 같은 문장이 나오면 API 호출 없이 캐시에서 바로 가져옵니다.
# 캐시를 초기화하려면 translation_cache.json 파일을 삭제하세요.
# ==========================================
...
# ==========================================
# [로컬 선처리] API 호출 없이 즉시 처리하는 부분
# ==========================================
# 아래 조건에 해당하면 Gemini API를 호출하지 않고 바로 처리합니다:
#   - 숫자만 있는 텍스트 (예: "100", "+3", "2d6")
#   - 순수 대문자 코드 (예: "STRENGTH", "DEX_MOD")
#   - 이미 한국어인 텍스트
#   - 용어집에 정확히 일치하는 단어
# 이렇게 하면 API 비용과 시간을 절약할 수 있습니다.
# ==========================================
...
# ==========================================
# [태그 보호] XML 태그가 번역 중 깨지지 않도록 보호
# ==========================================
# BG3 모드 XML에는 &lt;LSTag&gt; 같은 이스케이프된 태그가 있습니다.
# AI가 이걸 번역하거나 변형하면 게임에서 오류가 발생하므로,
# 번역 전에 플레이스홀더(__ESCAPED_TAG_1__ 등)로 바꿔두고
# 번역 후에 원래대로 복원합니다.
# ==========================================
...
# ==========================================
# [용어집 유틸] 용어집을 AI 프롬프트와 후처리에 적용하는 함수들
# ==========================================
...
# ==========================================
# [Gemini API] AI 번역 엔진 — 핵심 로직
# ==========================================
# 이 섹션이 실제로 Gemini AI에게 번역을 요청하는 부분입니다.
#
# get_system_instruction(): AI에게 보내는 번역 규칙/지시문
#   → 번역 스타일을 바꾸고 싶으면 이 함수의 프롬프트를 수정하세요
# extract_block_parts(): XML <content> 태그에서 텍스트만 추출
# call_gemini(): 실제 API 호출 (경량 포맷으로 토큰 절감)
# parse_response(): AI 응답을 파싱하여 번역 결과 추출
# ==========================================
...
# 2. 이미 한국어 / 숫자만 / 용어집 매칭 → 로컬 즉시 처리
# 3. 나머지를 청크로 나눠 Gemini API에 번역 요청
# 4. 번역 결과를 원본 XML에 반영하여 한글화된 XML 반환
#
# 번역 로직을 바꾸고 싶다면 이 함수를 수정하세요.
# ==========================================
    ...
    for loc_path in loc_folders:
        ...
        if not xml_files:
            ...
            if loca_files:
                ...
                print(f"      해결 방법:")
                print(f"        1) PAK 모드(BG3_AutoKorean_PAK_v2.2.py)를 사용하세요 (자동 변환)")
...
if __name__ == "__main__":
    print("=" * 60)
    print("   BG3 모드 자동 한글화 스크립트 v2.2 — 폴더 모드")
    print("=" * 60)
    print()
